...
Версия 0.9.0: M6 добавляет проверяемую YAML-онтологию и NetworkX-граф для семи приоритетных EUR/USD цепочек. 18 узлов и 18 связей экспортируются в JSON/GraphML; каждая недоступная переменная остаётся явным блокером, а стрелки не выдаются за доказанную причинность.
...
M4 даёт 267 выровненных строк. M5 использует 153 pre-event строки BLS/FOMC/ECB только для описательного zero-mean baseline. M6 описывает переходы event → surprise → expectations → rates → FX, но не создаёт отсутствующие признаки и не дублирует временные ряды в граф. Строгих PIT-цепочек пока 0; нулевой результат допустим.
