...
| Grouped (block) suite | v0.18 | DONE non-strict | Единица анализа = экономический блок: leave-one-block-out ablation (семья 4, Bonferroni 0.0125) + block permutation importance + within-block correlations + cross-reference с суммой single-LOO дельт v0.17. Блоки на текущей матрице: rates [2Y,10Y], energy [Brent,WTI], risk [VIX], fx_state [momentum]. Протокол: `config/grouped.yaml` (denn-grouped-1), CLI `denn-grouped`. **NULL подтверждён** (ни один блок не проходит); направленный near-miss rates 60d (p=0.33, gap +5.9e-05 — совместный вклад блока). Отчёт: `data/reports/denn_grouped.json`. World-state Tier A-план (TIPS, NFCI/ANFCI, TIC, H.4.1/ECB BS, relative equities, USD-ex-EUR, EA energy) — в DECISIONS.md; следующий шаг. |
...
8. Grouped (block) suite v0.18 (2026-09-26): пререгистрированная семья 4 leave-one-block-out ablations (rates [2Y,10Y], energy [Brent,WTI], risk [VIX], fx_state [momentum]), Bonferroni 0.0125 — **null подтверждён вторым сьютом**: ни один блок не проходит. Направленный near-miss: rates 60d dMSE +3.7e-05 (p = 0.33) при ОТРИЦАТЕЛЬНОЙ сумме single-LOO дельт 2Y+10Y (−2.2e-05) — блок несёт совместную информацию, которой нет ни у одной колонки (redundancy/synergy gap +5.9e-05); на 1d rates CI не пересекает 0 (13/17, p = 0.049). Диагностика: block permutation importance 1d — rates +3.5% MSE (максимум), within-block корреляции 2Y↔10Y r ≈ 0.6, Brent↔WTI r ≈ 0.86. Вывод: метод различает «блок как единицу» от «сумма одиночных», но на текущей 6-колоночной матрице значимого вклада нет — следующий шаг: расширение world-state (Tier A), а не новые эксперименты на той же матрице. Отчёт: `data/reports/denn_grouped.json`; детали — DECISIONS.md, секция 2026-09-26.
